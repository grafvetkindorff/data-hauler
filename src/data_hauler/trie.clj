  ...
  (let [str-seq (map str the-string)]
    ...
    (loop [remaining-chars str-seq
      ...
      (if (seq remaining-chars)
        (let [path-in-node (conj path (first remaining-chars))]
...
(def test-trie (-> {}
                   (insert-str-in-trie "john")
                   (insert-str-in-trie "john")
                   (insert-str-in-trie "johnny")
                   (insert-str-in-trie "joachim")
                   (insert-str-in-trie "is")
                   (insert-str-in-trie "isis")
                   (insert-str-in-trie "island")))
